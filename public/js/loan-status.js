(function () {
  ...
  function abitti2OutdatedWarnHtml() {
    const label = escapeHtml(t('loanAbitti2NewerAvailable'));
    return ` <span class="loan-abitti2-outdated" role="img" aria-label="${label}" title="${label}"><svg class="loan-abitti2-warn-triangle" viewBox="0 0 24 24" width="14" height="14" focusable="false" aria-hidden="true"><path d="M12 3 L22 20 H2 Z" fill="currentColor"/></svg></span>`;
  }
      ...
      const kindSuffix =
        row.primary_kind === 'other'
          ...
          : row.primary_kind === 'charger'
            ...
            : '';
      const abittiCell =
        escapeHtml(row.primary_abitti2_version_label || '—') +
        (row.primary_kind === 'computer' && row.primary_abitti2_outdated
          ? abitti2OutdatedWarnHtml()
          : '');
      tr.innerHTML = `
  ...
  function buildEquipmentRows(items, activeLoans) {
    ...
    items.forEach((it) => {
      ...
      const abittiOutdated = Boolean(it.abitti2Outdated);
      rows.push({
        kind: it.kind,
        id: it.id,
        avail,
  ...
  function renderList(rows) {
    ...
    rows.forEach((row) => {
      const tr = document.createElement('tr');
      const abittiListCell =
        escapeHtml(row.abittiDisplay) +
        (row.kind === 'computer' && row.abittiOutdated ? abitti2OutdatedWarnHtml() : '');
      tr.innerHTML = `
        <td>${escapeHtml(kindLabel(row.kind))}</td>
        <td>${row.nameHtml}</td>
        <td>${escapeHtml(row.brandDisplay)}</td>
        <td>${abittiListCell}</td>
