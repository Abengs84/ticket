...
/** Map trimmed label → rank (0 = newest per listAbitti2Versions order). */
function buildAbitti2LabelRankMap() {
  const versions = listAbitti2Versions();
  const rankByLabel = new Map();
  for (let i = 0; i < versions.length; i++) {
    const key = String(versions[i].label || '').trim();
    if (key && !rankByLabel.has(key)) rankByLabel.set(key, i);
  }
  return rankByLabel;
}

function isComputerAbitti2Outdated(kind, label, rankByLabel) {
  if (kind !== 'computer') return false;
  const lab = label != null ? String(label).trim() : '';
  if (!lab || rankByLabel.size === 0) return false;
  const rank = rankByLabel.get(lab);
...
function getLoanStatus() {
  ...
  const abitti2RankByLabel = buildAbitti2LabelRankMap();
  const activeLoans = active.map((row) => ({
    ...row,
    primary_abitti2_outdated: isComputerAbitti2Outdated(
      row.primary_kind,
      row.primary_abitti2_version_label,
      abitti2RankByLabel
    ),
  }));
  const items = assets.map((a) => {
    ...
    return {
      ...
      abitti2Outdated: isComputerAbitti2Outdated(
        a.kind,
        a.abitti2VersionLabel,
        abitti2RankByLabel
      ),
      available: !checkout,
      checkout: checkout
